...
// Config is the configuration for doc parser.
type Config struct {
...
func NewDocParser(ctx context.Context, config *Config) (*DocParser, error) {
	if config == nil {
		config = &Config{}
	}
	return &DocParser{ToPages: config.ToPages}, nil
}

func (pp *DocParser) Parse(ctx context.Context, reader io.Reader, opts ...parser.Option) (docs []*schema.Document, err error) {
